...
-- Nomes: vw_familias_limpa, vw_pessoas_limpa (para facilitar o agente de IA).
-- Base: cadu_raw + dicionário (d = família, p = pessoa).
-- Referência familiar: p.cod_parentesco_rf_pessoa = 1 (Pessoa Responsável pela Unidade Familiar - RF).
-- Uso: BI e agente; recriar a cada novo upload (executar após "Inserir lotes").
-- Se o layout do CSV mudar (novos campos), atualize este script e as tabelas auxiliares.
--
-- Se for colar este SQL dentro de um template literal em JavaScript (N8N Code node),
-- substitua nas regex: \d por \\d (ex.: '^\d{2}/' por '^\\d{2}/') para que o Postgres receba \d.
-- =============================================================================

-- Funções de normalização (reutilizáveis nas views)
...
-- Código familiar: TRIM + remove zeros à esquerda para cruzar fontes (ex.: 00123 = 123).
CREATE OR REPLACE FUNCTION norm_cod_familiar(t TEXT) RETURNS TEXT AS $$
  SELECT NULLIF(LTRIM(TRIM(COALESCE(t, '')), '0'), '');
$$ LANGUAGE SQL IMMUTABLE;

-- =============================================================================
-- Remover views antigas (recriadas a cada upload)
-- =============================================================================
...
-- =============================================================================
-- vw_familias_limpa: uma linha por família (DISTINCT ON por código familiar + IBGE)
-- Inclui d_nis_responsavel_fam = NIS da pessoa com cod_parentesco_rf_pessoa = 1 (RF).
-- Colunas d_* com TRIM, CAST e normalização (CPF entrevistador, CEP, telefone, datas, valores).
-- =============================================================================
CREATE VIEW vw_familias_limpa AS
SELECT DISTINCT ON (norm_cod_familiar(r.d_cod_familiar_fam), norm_text(r.d_cd_ibge))
    ...
    (SELECT norm_nis(r2.p_num_nis_pessoa_atual)
     FROM cadu_raw r2
     WHERE norm_cod_familiar(r2.d_cod_familiar_fam) = norm_cod_familiar(r.d_cod_familiar_fam)
...
FROM cadu_raw r
ORDER BY norm_cod_familiar(r.d_cod_familiar_fam), norm_text(r.d_cd_ibge), r.id;

-- =============================================================================
-- vw_pessoas_limpa: uma linha por pessoa (todas as linhas de cadu_raw)
-- Colunas p_* + chave de família (d_cod_familiar_fam, d_cd_ibge) com TRIM, CAST e normalização.
-- =============================================================================
