...
import { buildSibecUpload, SIBEC_BLOQUEADOS } from '@/lib/upload-sibec';
...
export async function POST(request: NextRequest) {
  ...
  const { truncateSql, insertStatements, rowCount } = buildSibecUpload(content, SIBEC_BLOQUEADOS);
  if (rowCount === 0) {
  ...
  try {
    ...
    return NextResponse.json({
      message: `SIBEC Bloqueados: ${rowCount} registro(s) carregado(s).`,
      rowCount,
      hint: 'Clique em "Atualizar todas as views" na Manutenção para repopular as consultas.',
    });
  } catch (e) {
    console.error('Upload sibec-bloqueados error:', e);
    return NextResponse.json({
